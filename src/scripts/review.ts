...
const noticeText=el('span');const retry=button('Retry',()=>void sync(),'rv-inline');status.append(noticeText,retry);panel.append(status);
...
nameInput.addEventListener('input',()=>{storage.set('seo-review-name',nameInput.value);updateAuthor();});
...
function notify(message='',error=false){noticeText.textContent=message;status.hidden=!message;status.classList.toggle('rv-error',error);retry.hidden=!error;const globalStatus=document.getElementById('site-mode-status')!;if(reviewActive){globalStatus.hidden=!(error&&!panelOpen);globalStatus.textContent=error&&!panelOpen?message+' Select Review to retry.':'';}}
...
canvas.addEventListener('pointerdown',e=>{if(!picking||busy||!e.isPrimary||e.button!==0||gesture)return;e.preventDefault();const target=hitAnchor(e.clientX,e.clientY);const start=docPoint(e);draftAnchor=null;selected=null;detail=null;detailSequence++;setURL(null);setPanel(false);gesture={pointerId:e.pointerId,start,end:start,clientX:e.clientX,clientY:e.clientY,target};canvas.setPointerCapture(e.pointerId);renderPins();});
...
function renderDetail(){if(!detail)return;const signature=JSON.stringify(detail);if(signature===detailSignature)return;detailSignature=signature;
 ...
 const t=detail.thread;cardTitle.textContent='Comment';panel.setAttribute('aria-label',`Comment ${t.id}: ${t.anchor_label}`);
...
function positionPopover(){
 if(!panelOpen)return;const anchor=draftAnchor||detail?.thread||list.find(t=>t.id===selected);if(!anchor){panel.hidden=true;return;}const box=geometryFor(anchor);if(!box){panel.hidden=true;return;}
 ...
 panel.hidden=false;const layout=attachedCard({x:pinX,y:pinY},panel.getBoundingClientRect(),{width:innerWidth,height:innerHeight});if(!layout){panel.hidden=true;return;}
