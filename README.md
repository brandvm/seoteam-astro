...
Open `https://brandvm.github.io/seoteam-astro/?mode=review` to review the page, or `?mode=browse` to use it normally. A single Browse / Review toolbar switches modes without reloading or changing the scroll position. Review opens the direct-placement experience: click for a pin, or drag a rectangle around an area. Click a saved pin to open its floating conversation. The old section-selection form, global sidebar, and separate Exit review control are removed. There is no global comment sidebar. Each conversation opens beside its pin, follows it during scrolling, and hides when the pin leaves the viewport. The toolbar steps between comments and filters Open / Resolved / All. New comments are placed directly on the page; earlier general page discussions remain available at a page-level pin.
